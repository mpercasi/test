...
public class ExceptionClienteHandler extends ResponseEntityExceptionHandler {
    ...
    @ExceptionHandler({SinCelularException.class})
    protected ResponseEntity<Object> sinCelulare(Exception ex, WebRequest request) {
        return handleExceptionInternal(ex, "El numero de celular esta equivocado", new HttpHeaders(), HttpStatus.BAD_REQUEST, request);
    }

    @ExceptionHandler({CorreoInexistenteException.class})
    protected ResponseEntity<Object> sinCorreo(Exception ex, WebRequest request) {
        return handleExceptionInternal(ex, "Correo no encontrado", new HttpHeaders(), HttpStatus.BAD_REQUEST, request);
    }

    @ExceptionHandler({CorreoYaExistenteException.class})
    protected ResponseEntity<Object> correoRepetido(Exception ex, WebRequest request) {
        return handleExceptionInternal(ex, "Este correo ya se encuentra registrado", new HttpHeaders(), HttpStatus.BAD_REQUEST, request);
    }

    @ExceptionHandler({ContraOUsuarioInvalido.class})
    protected ResponseEntity<Object> UsuarioInvalido(Exception ex, WebRequest request) {
        return handleExceptionInternal(ex, "El usuario es invalido", new HttpHeaders(), HttpStatus.BAD_REQUEST, request);
    }
}
